...
function reductionLabel(entry: HistoryEntry) {
  const reduction = Math.max(0, entry.compressionRatio) * 100;
  return `${Math.round(reduction)}% smaller`;
}
    ...
    <div className="h-full overflow-auto bg-[#0d1117] p-4">
      <div className="mx-auto flex min-h-[calc(100vh-72px)] max-w-6xl overflow-hidden rounded-[10px] border border-white/[0.08] bg-[#0d1117]">
        <aside className="flex w-[240px] shrink-0 flex-col gap-4 border-r border-white/[0.07] bg-[#0b0f15] p-4">
          <div>
            <p className="mb-2 text-[10px] uppercase tracking-[0.12em] text-white/30">History</p>
            <div className="rounded-lg border border-white/[0.08] bg-white/[0.04] p-3">
              <div className="mb-2 flex items-center gap-2 text-xs font-medium text-white"><History size={14} className="text-[#4fc3a1]" /> Recent exports</div>
            </div>
          </div>

          <div className="grid gap-2">
            <div className="rounded-lg border border-white/[0.08] bg-white/[0.035] p-3">
              <p className="text-[10px] uppercase tracking-[0.12em] text-white/30">Exports</p>
              <p className="mt-1 text-lg font-semibold text-white">{items.length}</p>
            </div>
            <div className="rounded-lg border border-white/[0.08] bg-white/[0.035] p-3">
              <p className="text-[10px] uppercase tracking-[0.12em] text-white/30">Space saved</p>
        ...
        <main className="min-w-0 flex-1 bg-[#0d1117] p-6">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <p className="text-[11px] uppercase tracking-[0.12em] text-white/35">History</p>
              <h1 className="mt-1 text-lg font-medium text-white">Recent exports</h1>
            </div>
          ...
          {loading ? (
            <div className="grid min-h-[360px] place-items-center rounded-lg border border-white/[0.08] bg-white/[0.02] text-white/45">
              <div className="flex items-center gap-2 text-xs"><Loader2 size={15} className="animate-spin text-[#4fc3a1]" /> Loading history...</div>
            </div>
          ) : items.length === 0 ? (
            <div className="grid min-h-[360px] place-items-center rounded-lg border border-dashed border-white/[0.08] bg-white/[0.02] p-8 text-center">
              <div>
                ...
                <p className="text-sm font-medium text-white">No successful exports yet</p>
                <p className="mx-auto mt-2 max-w-md text-[11px] leading-relaxed text-white/40">Completed exports will appear here briefly, capped at the latest 15 files.</p>
              </div>
            </div>
          ) : (
                  ...
                  <div className="flex gap-4 p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 grid gap-2 text-[11px] text-white/45 sm:grid-cols-4">
                        <div className="rounded-lg bg-white/[0.035] p-2"><p className="text-white/30">Before</p><p className="mt-0.5 text-white">{formatBytes(item.inputSizeBytes)}</p></div>
                        <div className="rounded-lg bg-white/[0.035] p-2"><p className="text-white/30">After</p><p className="mt-0.5 text-white">{formatBytes(item.outputSizeBytes)}</p></div>
                        <div className="rounded-lg bg-white/[0.035] p-2"><p className="text-white/30">Duration</p><p className="mt-0.5 text-white">{formatSeconds(item.durationSeconds)}</p></div>
                        <div className="rounded-lg bg-white/[0.035] p-2"><p className="text-white/30">Created</p><p className="mt-0.5 truncate text-white">{formatDate(item.createdAt)}</p></div>
                      </div>
                    </div>
                  </div>

                  <div className="flex items-center justify-between gap-3 border-t border-white/[0.07] bg-white/[0.025] px-4 py-2">
                    <div className="flex min-w-0 items-center gap-2 text-[11px] text-white/35"><Clock size={13} /> <span className="truncate">Source: {fileName(item.inputPath)}</span></div>
                    <button
                      type="button"
