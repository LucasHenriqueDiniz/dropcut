...
// Only the steps that have something real to show carry a screenshot.
const steps = [
  {
    titleKey: 'onboarding.simpleTitle',
    bodyKey: 'onboarding.simpleBody',
    image: '/tour/drop-example.png',
  },
  {
    titleKey: 'onboarding.contextMenuTitle',
    bodyKey: 'onboarding.contextMenuBody',
    image: '/tour/context-example.png',
  },
  {
    titleKey: 'onboarding.nerdTitle',
    bodyKey: 'onboarding.nerdBody',
    image: null,
  },
] satisfies { titleKey: TranslationKey; bodyKey: TranslationKey; image: string | null }[];

const CONTEXT_MENU_STEP = 1;
    ...
    <Modal open={open} className="max-w-lg">
      {step.image ? (
        <img
          src={step.image}
          alt=""
          className="mb-5 h-44 w-full rounded-xl border border-white/10 bg-black/30 object-cover"
          draggable={false}
        />
      ) : (
        <div className="mb-5 grid size-12 place-items-center rounded-xl bg-[#4fc3a1]/12 text-[#4fc3a1]">
      ...
      <h2 className="mt-1 text-xl font-semibold text-white">{t(step.titleKey)}</h2>
