...
export function PresetSelector({ value, presets, onChange, onToggleVisible }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        {visiblePresets.map((preset) => (
          ...
          >
            ...
            <p className="text-xs text-slate-500">~ {preset.targetMiB} MiB</p>
      ...
      >
        <Settings2 size={13} /> Manage visible presets
      </button>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur-sm" onMouseDown={() => setManageOpen(false)}>
          <div className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-950 p-5 shadow-2xl shadow-black/50" onMouseDown={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Presets</p>
                <h3 className="mt-1 text-lg font-semibold text-white">Visible in editor</h3>
                <p className="mt-1 text-sm text-slate-500">Choose which presets appear in the sidebar quick picker.</p>
              </div>
