  ...
  return (
    <div className="relative size-24 shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="mt-1 text-[9px] uppercase tracking-[0.16em] text-white/35">of 100</span>
      </div>
    </div>
  );
}
...
export function ProgressDialog({ open, status, progress, outputPath, inputSizeBytes, outputSizeBytes, onClose, onOpenFolder, compact = false }: Props) {
  ...
  const progressTitle = isDone ? 'Export complete' : isError ? 'Export failed' : percent < 30 ? 'Preparing export…' : percent < 90 ? 'Processing video…' : 'Almost ready…';
  const stageLine = isDone
    ? 'Your compressed video is ready.'
    : isError
      ? status
      : percent < 30
        ? 'Starting FFmpeg and preparing output…'
        : percent < 90
          ? 'Encoding frames and optimizing the container…'
          : 'Finalizing and writing the file…';
          ...
          <div className="mb-4 flex items-start justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[10px] uppercase tracking-[0.16em] text-white/30">Exporting video</p>
                <p className="mt-1 text-sm font-semibold leading-tight text-white">{isError ? 'Export failed' : isDone ? 'Export complete' : currentStep}</p>
              </div>
            </div>
            {(isDone || isError) && onClose && (
              <button type="button" onClick={onClose} className="grid size-7 place-items-center rounded-lg text-white/35 transition hover:bg-white/10 hover:text-white" aria-label="Close progress">
                <X size={14} />
              </button>
            )}
          </div>
          ...
          {hasSizeSummary && (
            <div className="mt-4 grid gap-2 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-emerald-300/20 bg-emerald-400/10 px-3 py-2">
                <p className="text-[10px] uppercase tracking-[0.12em] text-emerald-100/70">After</p>
          ...
          {isDone ? (
            <div className="relative">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.16em] text-white/35">Export complete</p>
            ...
            <>
              ...
              <div className="relative grid gap-5 sm:grid-cols-[140px_1fr] sm:items-center">
                <div className="mx-auto rounded-full p-2" aria-label={`Export progress ${percent}%`}>
